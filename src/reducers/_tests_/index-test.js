  ...
  it("should handle sign pressed when a number is picked", () => {
    expect(
      calculate(
        ...
        actions.operate(operations.SUBTRACT)
      )
    ).toEqual({
      stack: Stack([
  ...
  it("should handle number pressed after an operation", () => {
    expect(
      calculate(
        ...
        actions.pickNumber("4")
      )
    ).toEqual({
      stack: Stack([
        { action_type: types.PICK_NUMBER, value: "4" },
        { action_type: types.OPERATE, value: operations.SUBTRACT },
        { action_type: types.PICK_NUMBER, value: "57" }
      ])
    });
  ...
  it("should handle a second sign being pressed (i.e. sign change)", () => {
    expect(
      calculate(
        ...
        actions.operate(operations.ADD)
      )
    ).toEqual({
      stack: Stack([
        { action_type: types.OPERATE, value: operations.ADD },
        { action_type: types.PICK_NUMBER, value: "57" }
      ])
    });
  });
  ...
  it("should clear the stack", () => {
    expect(
      calculate(
        ...
        actions.operate(operations.CLEAR)
      )
    ).toEqual({
      stack: Stack()
    });
  ...
  it("should negate a number", () => {
    expect(
      calculate(
        ...
        actions.operate(operations.NEGATE)
      )
    ).toEqual({
  ...
  it("should calculate the percent for a number", () => {
    expect(
      calculate(
        ...
        actions.operate(operations.PERCENT)
      )
    ).toEqual({
